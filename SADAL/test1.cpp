...
bool Prog(istream& in, int& line)
{
	...
	procName = tok.GetLexeme(); // check for proc name
...
bool PrintStmts(istream& in, int& line)
{
	...
	// Output evaluated data (do addition or subtraction ect)
...
bool Name(istream& in, int& line, int sign, Value& retVal)
{
	...
	else
	{
		...
		switch (vt)
		{
		case VINT:    retVal = Value(valMap[varName].GetInt() * sign); break;
		case VREAL:   retVal = Value(valMap[varName].GetReal() * sign); break;
		case VBOOL:   retVal = valMap[varName]; break;
		case VCHAR:   retVal = valMap[varName]; break;
		case VSTRING: retVal = valMap[varName]; break;
